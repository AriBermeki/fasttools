...
def create_task(coroutine: Awaitable, *, name: str = 'unnamed task', loop:asyncio.AbstractEventLoop) -> asyncio.Task:
...
def create_lazy(coroutine: Awaitable, *, name: str, loop:asyncio.AbstractEventLoop) -> None:
    ...
    def finalize(name: str) -> None:
        ...
        if name in lazy_tasks_waiting:
            create_lazy(lazy_tasks_waiting.pop(name), name=name, loop=loop)
    task = create_task(coroutine, name=name, loop=loop)
